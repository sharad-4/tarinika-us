    ...
    if (cart.item_count > 0) {
      $.ajax({
        ...
        success: function (html) {
          ...
          const tpoints = $(html).find('.cart__points');
          const ctotal = $(html).find('.cart__total-amount');
          const itemsaving = $(html).find('.cart__item-saving')
          const subtotalamount = $(html).find('.cart__subtotal-amount');

          $('.cart__subtotal-container').replaceWith(subtotal);
          $('.cart__points').replaceWith(tpoints);
          $('.cart__total-amount').replaceWith(ctotal);
          $('.cart__subtotal-amount').replaceWith(subtotalamount);
          $('.cart__item-saving').replaceWith(itemsaving);
